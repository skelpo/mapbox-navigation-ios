...
class NavigationServiceTests: XCTestCase {
    ...
    // Test whether out-of-band calls to methods in RouteController which in turn call Navigator.getStatusForTimestamp(_:)
    // do not cause any side effects (e.g. skipped voice or banner instructions, intersection index).
    // Fixture which is used in this test contains 14 voice instructions. It is expected that even after multiple calls to
    // Navigator.getStatusForTimestamp(_:) sequence of these instructions will not change and RouteController will always return
    // expected voice or banner instruction.
    func testOutOfBandSpokenInstructions() {
        let routeOptions = NavigationRouteOptions(coordinates: [
            CLLocationCoordinate2D(latitude: -0.133716, longitude: 51.510057),
            CLLocationCoordinate2D(latitude: -0.137277, longitude: 51.513551),
        ])

        let routeData = Fixture.JSONFromFileNamed(name: "route-with-voice-instructions")
        let decoder = JSONDecoder()
        decoder.userInfo[.options] = routeOptions
        let route: Route? = try? decoder.decode(Route.self, from: routeData)
        
        var voiceInstructionsArray = [
            "Head east on Coventry Street (A4), then turn right onto Haymarket (A4)",
            "In 700 feet, turn right onto Charles II Street (B327)",
            "Turn right onto Charles II Street (B327), then turn right onto Regent Street St James\'s (A4)",
            "Turn right onto Regent Street St James\'s (A4)",
            "In 1000 feet, go straight onto Regent Street (A4201)",
            "Go straight onto Regent Street (A4201), then turn right onto Air Street",
            "Turn right onto Air Street, then turn right onto Brewer Street",
            "Turn right onto Brewer Street",
            "In 500 feet, turn left onto Lexington Street",
            "Turn left onto Lexington Street",
            "In 700 feet, turn left onto Broadwick Street",
            "Turn left onto Broadwick Street, then turn right onto Dufour\'s Place",
            "TurnrightontoDufour\'s Place, then you will arrive at your destination",
            "YouhavearrivedatDroppedPin#1,ontheleft"
        ]
        
        var bannerInstructionsArray = [
            "Haymarket",
            "Charles II Street #1",
            "Charles II Street #2",
        ...
        if let route = route {
            XCTAssertEqual(route.legs[0].steps.count, 11, "Unexpected number of steps.")
            ...
            // Iterate over each step and simulate movement along the route.
            route.legs[0].steps.enumerated().forEach {
                let stepCoordinates = $0.element.shape!.coordinates
                let stepLocations: [CLLocation] = stepCoordinates.enumerated().map {
                    let currentCoordinate = $0.element
                ...
                stepLocations.enumerated().forEach { 
                    ...
                    // Attempt to snarf instructions by calling methods which internally call Navigator.getStatusForTimestamp(_:) multiple times.
                    let outOfBandCallsCount = 5
